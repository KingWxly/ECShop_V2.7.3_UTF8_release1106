(function($, window, document, undefined) {
	...
	var BsBanner = function(ele, opt) {
		this.$ele = ele,
			this.defaults = {
				...
				'nav': false, //开启小选项圈，可选择查看第几张图片
				'window_resize': true, //开启屏幕大小改变时调整banner
				'banner_time': 5000
			},
	...
	BsBanner.prototype = {
		...
		ready: function() {
			...
			if (_options.banner_width) $(_banner).css({
				"width": _options.banner_width
			});
			...
			_boms.each(function() {
				$(this).css({
					"width": _width + "px",
					"height": _options.banner_height ? _options.banner_height + "px" : "auto",
					"float": "left"
				});
			});
		...
		windowResize: function() {
			...
			if (_options.banner_width) _options.banner.css({
				"width": _options.banner_width
			});
